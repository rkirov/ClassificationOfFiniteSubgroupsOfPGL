...
\\
Unless otherwise stated, the letters $\alpha, \beta, \gamma, \delta, \sigma, \gamma$, and $\sigma$ will denote elements of $F$ and $\delta$ and $\rho$ elements of $F^\times$, where $F^\times$  are the non-zero elements of $F$.
...
\begin{lemma}[Closure $D$ under multiplication]
...
\begin{lemma}[Closure $S$ under multiplication]
...
\begin{remark}
    To define the subgroups $D$, $S$ and $L$ in Lean. One has to define what the underlying set is, the \texttt{carrier}, 
    prove that the set is closed under multiplication \texttt{mul_mem'}, prove that the set contains the identity element of the group, \texttt{one_mem'}, 
    and finally one must show that the group is closed under the inversion operator $\cdot^{-1}$, \texttt{inv_mem'}.
\end{remark}
...
\begin{remark}
    ...
    If either $D$ or $S$ were normal in $\SL_2(F)$ we would be able to use \texttt{normal_mul}. However, given neither $D$ nor $S$ are normal in $\SL_2(F)$ slightly more work is needed to show this.
...
\begin{remark}[Subgroup of subgroups in Lean]
...
\begin{align*} \pi(d_\delta s_\sigma d_\rho s_\gamma) &= \pi(d_\delta d_\rho t_\sigma s_\gamma) \tag{where $\sigma = \sigma \rho^{2}$ by Lemma \ref{6.1}}
\\ &= d_\delta d_\rho
...
\section[Conjugacy of the Elements of $L$]{Conjugacy of the Elements of $\SL_2(F)$}
...
\begin{lemma}[Upper triangularizability criteria]
    ...
    \leanok
    A matrix $M \in \textrm{Mat}_2(F)$ is triangularizable if and only if there exists an invertible matrix $C \in \GL_2(F)$ such that the bottom left entry of
    $C M C^{-1}$ is zero.
\end{lemma}
...
yields a diagonal matrix (see the Lean code!)
\end{proof}
...
The proof of the lemma is extracted from Christopher Butler's expostion uses the Jordan Normal Form theorem

At the time of writing, this theorem is still not yet in Mathlib, there are a few reasons why: the classification of nilpotent endomorphisms has not yet been done and the classification of semisimple endomorphisms has not been completed yet either.

Furthermore, it is not clear which is the cleanest way to formalise this yet; because ideally this theorem will be available in maximal generality considering it's importance for classification problems.

I initally set out to prove the Jordan Normal form for the special case of $2 \times 2$ matrices the "morally" correct way, which uses the wonderful abstract machinery of linear alegbra. In particular, understanding the eigenspace and generalized eigenspace of a the associated endomorphism to the matrix. 

This will be the way the more general theorem should be done to my understanding. But this task turned out to be essentially equivalent to proving the general case and then use it for the special case.

Given my task has a fair amount of constraints, $2 \times 2$ matrices with determinant one over an algebraically closed field. I eventually opted after Prof. Kevin Buzzard's advice to simply classify matrices by finding suitable matrices by which to conjugate a given matrix.
\end{remark}
...
\begin{proposition}
\label{normalizer_subgroup_S_leq_L}
% find dependencies - move proof - rename
\lean{normalizer_subgroup_S_leq_L}
\leanok
 For any subgroup $S_1 \leq S$ with order greater than 1, we have that the normalizer $N_{\SL_2(F)}(S_1) \subset L$.
\end{propostion}
...
\begin{proof}
(i) Let $s_\sigma$ be an arbitary element of $S_1$ with $\sigma \neq 0$. To determine the normaliser of $S_1$ in $\SL_2(F)$we consider which $x \in \SL_2(F)$ satisfy $x s_\sigma x^{-1} \in S_1$.
\begin{align*} x s_\sigma x^{-1} &= \begin{bmatrix} \alpha & \beta \\ \gamma & \delta \end{bmatrix} \begin{bmatrix} 1 & 0 \\ \sigma & 1 \end{bmatrix} \begin{bmatrix} \delta & \minus \beta \\ \minus \gamma & \alpha \end{bmatrix}
\\[1.5ex] &= \begin{bmatrix} \alpha & \beta \\ \gamma & \delta \end{bmatrix} \begin{bmatrix} \delta & \minus \beta \\ \delta \sigma - \gamma & \alpha - \beta \sigma \end{bmatrix}
\\[1.5ex] &= \begin{bmatrix} \alpha \delta - \beta \gamma + \beta \delta \sigma & \minus \beta^2  \sigma \\ \delta^2 \sigma & \alpha \delta - \beta \gamma - \beta \delta \sigma \end{bmatrix}.
\end{align*}
Since $x s_\sigma x^{-1} \in S_1$ we have $\minus \beta^2  \sigma = 0$ and since $\sigma \neq 0$, we have $\beta = 0$. Since $s_\sigma$ was chosen arbitrarily, any element which normalises $S_1$ is a lower diagonal matrix and is therefore in $H$ by (\ref{Hlowertri}). Thus $N_{\SL_2(F)}(S_1) \subset H$ as required. \\
\\
(ii) To determine the centraliser of $s_\sigma$ in $L$, we consider which $y \in \SL_2(F)$satisfy $y s_\sigma = s_\sigma y$ for an arbitrarily chosen $s_\sigma$, with $\sigma \neq 0$. \\
\vspace{-0.5mm}
...
\begin{align*} 
    xt_\sigma = s_\sigma x \iff \minus(x s_\sigma) = \minus (s_\sigma x) \iff x(\minus s_\sigma) = (\minus s_\sigma)x.
\end{align*}
