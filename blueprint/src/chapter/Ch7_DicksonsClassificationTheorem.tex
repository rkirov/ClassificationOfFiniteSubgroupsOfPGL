...
\end{lemma}
\begin{proof} 
    The approach here is proof by contradiction, so we begin by assuming that $Q \vartriangleleft G$. Thus $N_G(Q) = G$ and $N_G(K) \subset N_G(Q)$. Consider the natural homomorphism of $N_G(Q)$ onto $N_G(Q)/Q$,
...
\begin{footnotesize}
\begin{verbatim}
noncomputable abbrev GaloisField.polynomial 
    (p : ℕ) [hp : Fact (Nat.Prime p)] (n : ℕ+) : (ZMod p)[X] := 
    X ^ p ^ n.val - X
\end{verbatim}
\end{footnotesize}
...
\begin{lemma}
    ...
    \lean{GaloisField.splits_of_dvd}
    \leanok
...
\begin{definition}[Field extension of $\mathbb{F}_{q^2}$ of $\mathbb{F}_{q}$]
    \label{GaloisField.algHom_of_dvd}
    \uses{GaloisField.splits_of_dvd}
    \lean{GaloisField.algHom_of_dvd}
    \leanok
...
\begin{footnotesize}
\begin{verbatim}
noncomputable  def GaloisField.algHom_of_dvd (hn : n ∣ m) : 
    GaloisField p n →ₐ[ZMod p] GaloisField p m :=
      Polynomial.SplittingField.lift _ (splits_of_dvd hn)
\end{verbatim}
\end{footnotesize}

From this $\mathbb{F}_p$-algebra we can now define the embedding of $\SL_2(\mathbb{F}_{q})$ into $\SL_2(\mathbb{F}_{q^2})$

\begin{definition}
    \label{SL2_monoidHom_SL2}
    \uses{GaloisField.algHom_of_dvd}
    \lean{SL2_monoidHom_SL2}
    \leanok
...
\begin{definition}[Group presentation for the dihedral group]
...
\begin{theorem}[Class I]
    ...
    \lean{dicksons_classification_theorem_class_I}
    \leanok
...
\end{lemma}
\begin{proof}
\uses{SpecialSubgroups.exists_unique_orderOf_eq_two}
...
\begin{footnotesize}
\begin{verbatim}
theorem FLT_classification_fin_subgroups_of_PGL2_over_AlgClosure_ZMod 
    {p : ℕ} [Fact (Nat.Prime p)] (𝕂 : Type*)
    [Field 𝕂] [CharP 𝕂 p] [Finite 𝕂]
    (G : Subgroup (PGL (Fin 2) (AlgebraicClosure (ZMod p)))) 
    [hG : Finite G] :
    IsCyclic G
    ∨
