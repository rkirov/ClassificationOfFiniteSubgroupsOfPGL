...
\begin{definition}
  ...
  \lean{ConjClassOfSet}
  \leanok
  Let $G$ be a subgroup of $\SL_2(F)$ then the set $\mathcal{C}_i = \{ x A_i x^{-1} : x \in G \}$ is called the \textbf{conjugacy class} of $A_i \in \mathfrak{M}$.
\end{definition}
...
\begin{definition}
  ...
  \leanok
  Let $G$ be a subgroup of $\SL_2(F)$ then let the noncenter conjugacy class of a maximal abelian subgroup be defined by
  
  \[
  \mathcal{C}_i* = \left\{x A_i^* x^{-1} \; | \; x \in G \right\}
   \]
\end{definition}
...
\begin{definition}[Conjugacy class of noncenter subgroup]
\label{C}
\lean{C}
\leanok
For some $A_i \in \mathfrak{M}$ and $A_i^* \in \mathfrak{M}^*$. Then let,
\begin{align*} 
  C_i = \bigcup_{x \in G} x A_i x^{-1}, \quad \text{and}
\end{align*}
\end{definition}
...
\begin{definition}
  ...
  \lean{noncenter_C}
  \leanok
  Similary, the noncenter conjugacy class of a subgroup is
\begin{align*}
  C_i^* = \bigcup{x \in G} x A_i^* x^{-1}.
\end{align*}
\end{definition}
...
\uses{MaximalAbelianSubgroupsOf, noncenter_C, noncenter_MaximalAbelianSubgroupsOf, noncenter_ConjClassOfSet}
...
\label{lift_MaximalAbelianSubgroupsOf}
\uses{MaximalAbelianSubgroupsOf}
\lean{lift_MaximalAbelianSubgroupsOf}
\leanok
 Let $G$ be a finite subgroup of $\SL_2(F)$, then the relation $\sim$ on the set of maximal abelian subgroups of $G$, $\mathfrak{M}$ defined by
 \[
...
\begin{proof}
 Define a relation $\sim$ on  $\mathfrak{M}^*$  as follows:
...
Now that we have set up the equivalence relation on maximal abelian subgroups we proceed to set up the maximal abelian class equation.


% REFACTOR TO noncenter_MaximalAbelianSubgroupsOf lift_noncenter_MaximalAbelianSubgroupsOf - No ocurrences of MaximalAbelianSubgroupsOf
% or define bijection between sets and lifts.
