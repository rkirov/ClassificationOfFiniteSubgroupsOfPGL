...
\begin{example}[Proving and formalising the sum of the first $n$ odd integers]
    ...
    define the corresponding program which constructs the term, but often it is more intuitive to enter what is known as \texit{tactic mode}.
    ...
    Finally, \texttt{rfl} proves any goal that is true by reflexivity of the given relation; in this case, we finish proving the goal by reflecivity of the equality relation.

    Theorems and lemmas in Lean are given an identifier by which to access through, for example: 
\end{example}
