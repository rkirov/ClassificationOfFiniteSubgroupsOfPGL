...
--   (A : MaximalAbelianSubgroupsOf G) : ConjClassOf G A → G ⧸ ((normalizer (A.val.subgroupOf G : Set ↥G))) :=
--   fun conj_A => Quot.mk ⇑(QuotientGroup.leftRel ((normalizer (A.val.subgroupOf G : Set ↥G))))
