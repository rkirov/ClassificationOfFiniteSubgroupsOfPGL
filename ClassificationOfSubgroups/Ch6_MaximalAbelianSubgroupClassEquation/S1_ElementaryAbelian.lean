...
lemma dvd_card {G : Type*} [Group G] {p : ℕ} {H : Subgroup G}
  ...
  have order_eq_p : orderOf (⟨h, h_in_H⟩ : H) = p := by
    apply hH.right ⟨h, h_in_H⟩
    simp [h_ne_one]
  rw [← order_eq_p]
  let inst : Fintype (H :) := Fintype.ofFinite ↥H
  have order_dvd_card := orderOf_dvd_card (x := (⟨h, h_in_H⟩ : H))
...
lemma primeFac_eq {G : Type*} [Group G] (p : ℕ)
  ...
  · by_contra! h
    ...
    simp at q_dvd_card
    have hq' : Fact (Nat.Prime q) := ⟨hq⟩
    obtain ⟨x, order_eq_q⟩ := exists_prime_orderOf_dvd_card q q_dvd_card
...
lemma IsPGroup {G : Type*} [hG : Group G] (p : ℕ) (hp : Nat.Prime p)
  ...
  IsPGroup p H := by
  have hp' : Fact (Nat.Prime p) := ⟨hp⟩
  rw [IsPGroup.iff_card]
  have : Nat.primeFactors (Nat.card (H :)) = {p} := primeFac_eq p hp H hH bot_lt_H
